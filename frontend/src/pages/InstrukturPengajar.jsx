...
export default function InstrukturPengajar() {
  ...
  const [kriteria, setKriteria] = useState(
    searchParams.get('kategori') || searchParams.get('kriteria') || 'Instruktur Pengajar'
  );
  ...
  useEffect(() => {
    const instance = L.map('instruktur-pengajar-map', { zoomControl: false }).setView([-6.9, 107.2], 7);
        ...
        .bindPopup(`
          <div style="min-width:200px">
            ...
            <a href="https://www.google.com/maps/search/?api=1&query=${e.lat},${e.lng}" 
              ...
              <span style="font-size:14px">📍</span> ${t('instruktur.open_in_google_maps', 'Buka di Google Maps')}
            </a>
          </div>
        `);
      marker.on('click', () => {
  ...
  const handleReset = () => {
    setKeyword('');
    setLocation('');
    setKriteria('Instruktur Pengajar');
    setOrder('latest');
        ...
        <section className="border-r border-gray-200 overflow-y-auto max-h-[calc(100vh-72px)] md:max-h-[calc(100vh-80px)]">
          ...
          {!loading && sortedExperts.length === 0 && (
            <div className="text-center text-gray-500 text-sm py-16 px-6">
              Tidak ada instruktur pengajar yang cocok.
              <br />
